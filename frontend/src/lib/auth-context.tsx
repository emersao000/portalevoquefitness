...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Auto-renew session every 30 minutes to keep it alive
  useEffect(() => {
    const renewSession = async () => {
      const sessionToken = localStorage.getItem("auth_session_token");
      const expiresAt = localStorage.getItem("auth_expires_at");
      if (!sessionToken || !expiresAt || !user) return;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.expires_at) {
            ...
            console.debug("[AUTH] ✓ Session renewed, new expiry:", data.expires_at);
          }
        }
      } catch (e) {
        console.debug("[AUTH] Session renewal failed (will retry):", e);
      }
    };

    // Renew every 30 minutes
    const intervalId = setInterval(renewSession, 30 * 60 * 1000);
  ...
  const checkExistingSession = async (): Promise<boolean> => {
    try {
      ...
      if (!sessionToken || !expiresAt) {
        console.debug("[AUTH] No session found in localStorage");
        setUser(null);
        return false;
      }
      ...
      if (now >= expiration) {
        console.debug("[AUTH] Session expired (client-side check)");
      ...
      // Restore user from localStorage BEFORE validating with backend
      // This ensures a fast load even if backend is slow/unavailable
      ...
      let userData: User;
      try {
        userData = JSON.parse(userDataRaw) as User;
        if (userData.bi_subcategories === undefined) {
          userData.bi_subcategories = null;
        }
        // Restore user immediately so app loads fast
        setUser(userData);
        console.debug("[AUTH] ✓ User restored from localStorage immediately");
      } catch (e) {
        console.error("[AUTH] Failed to parse user data from localStorage:", e);
        localStorage.removeItem("evoque-fitness-auth");
        setUser(null);
        return false;
      }

      // Validate session with backend in background (non-blocking for initial load)
      try {
        ...
        if (!response.ok) {
          // Backend error (5xx) or network issue — keep session, don't force logout
          console.warn(
            "[AUTH] Session validation returned non-ok status:",
            response.status,
            "- keeping session to avoid false logout"
          );
          return true;
        }
        ...
        if (validationData && validationData.is_valid === false) {
          // Explicitly invalid — session was revoked or doesn't exist in DB
          console.debug("[AUTH] Session explicitly invalidated by backend");
          localStorage.removeItem("auth_session_token");
          localStorage.removeItem("auth_expires_at");
          localStorage.removeItem("evoque-fitness-auth");
          setUser(null);
        ...
        console.debug("[AUTH] ✓ Session validated by backend");
      } catch (fetchError: any) {
        if (fetchError?.name === "AbortError") {
          console.warn("[AUTH] Session validation timed out — keeping session");
        } else {
          console.warn(
            "[AUTH] Session validation network error — keeping session:",
            fetchError
          );
        }
        // Network failure: keep existing session, don't force logout
        return true;
      }
      ...
      // On unexpected error, don't clear session — let user stay logged in
      const userDataRaw = localStorage.getItem("evoque-fitness-auth");
      if (userDataRaw) {
        try {
