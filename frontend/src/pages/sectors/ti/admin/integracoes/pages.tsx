...
import { Trash2, Plus, Edit2, Grid3x3, List, Zap, Package, X, Save } from "lucide-react";

// ─── Edit Problema Dialog ─────────────────────────────────────────────────────
...
interface EditProblemaDialogProps {
  problema: { id: number; nome: string; prioridade: string; requer_internet: boolean };
  slaConfigs: SlaConfiguracao[];
  onClose: () => void;
  onSaved: () => void;
}

function EditProblemaDialog({ problema, slaConfigs, onClose, onSaved }: EditProblemaDialogProps) {
  const [nome, setNome] = useState(problema.nome);
  const [prioridade, setPrioridade] = useState(problema.prioridade);
  const [requer, setRequer] = useState(problema.requer_internet);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Prioridades disponíveis: do SLA se houver, senão fallback
  const prioridades = slaConfigs.length > 0
    ? slaConfigs.filter(c => c.ativo).map(c => c.prioridade)
  ...
  async function handleSave() {
    ...
    setError(null);
    try {
      const res = await apiFetch(`/problemas/${problema.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ nome: nome.trim(), prioridade, requer_internet: requer }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error(data?.detail || "Erro ao atualizar problema");
      }
      onSaved();
      onClose();
    } catch (e: any) {
      setError(e?.message || "Não foi possível atualizar o problema.");
    } finally {
      setSaving(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-md mx-4 p-6 space-y-5">
        <div className="flex items-center justify-between">
          <div>
            <h3 className="font-semibold text-base">Editar Problema</h3>
            <p className="text-xs text-muted-foreground mt-0.5">Altere os dados do problema cadastrado</p>
          </div>
        ...
        <div className="space-y-3">
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">Nome do Problema</label>
            <input
              className="w-full rounded-md bg-background border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
              value={nome}
          ...
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">
              Prioridade
              {slaConfigs.length > 0 && (
                <span className="ml-1.5 text-xs text-primary/60">(definida pelo SLA)</span>
              )}
            </label>
            <select
              className="w-full rounded-md bg-background border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/30"
            ...
            >
              ...
              {/* Se a prioridade atual não está na lista, mostra mesmo assim */}
        ...
        {error && (
          <div className="text-xs px-3 py-2 rounded-md bg-red-100/50 text-red-700 dark:bg-red-900/20 dark:text-red-300">
            {error}
          </div>
        )}
...
function ProblemaCard({
  ...
  return (
    <>
      {editing && (
        <EditProblemaDialog
          problema={{ id, nome, prioridade, requer_internet: requerInternet }}
      ...
      <div className="rounded-lg border border-border/60 bg-card overflow-hidden hover:shadow-md hover:border-primary/20 transition-all">
        <div className="px-4 py-3 border-b border-border/60 bg-muted/30 flex items-center gap-2 justify-between">
          ...
          <div className="flex items-center gap-1.5">
            <span
              className={`inline-flex items-center gap-1 text-xs font-medium rounded-full px-2.5 py-1 whitespace-nowrap ${priorityColor[prioridade] || "bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-300"}`}
            >
              {prioridade}
            </span>
            <button
        ...
        <div className="p-4">
          ...
          {!requerInternet && (
            <div className="text-xs text-muted-foreground">Sem requisitos especiais</div>
          )}
        </div>
      </div>
    </>
...
function ListProblemaItem({ problema, slaConfigs, onDelete, onRefresh }: { problema: { id: number; nome: string; prioridade: string; requer_internet: boolean }; slaConfigs: SlaConfiguracao[]; onDelete: (id: number) => void; onRefresh: () => void }) {
  const [deleting, setDeleting] = useState(false);
  const [editing, setEditing] = useState(false);
...
export function AdicionarBanco() {
  ...
  const [saving, setSaving] = useState(false);
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");
  const [slaConfigs, setSlaConfigs] = useState<SlaConfiguracao[]>([]);

  // Prioridades para o formulário de criação
  const prioridades = slaConfigs.length > 0
    ? slaConfigs.filter(c => c.ativo).map(c => c.prioridade)
    : ["Normal", "Alta", "Crítica", "Baixa"];
  ...
  useEffect(() => {
    load();
    // Busca configurações de SLA para popular os selects de prioridade
    apiFetch("/sla/configuracoes")
      .then((r) => r.ok ? r.json() : Promise.reject())
      .then((data: SlaConfiguracao[]) => Array.isArray(data) && setSlaConfigs(data))
      .catch(() => {}); // silently fail - usa fallback hardcoded
  }, []);
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      setNome("");
      setPrioridade(prioridades[0] || "Normal");
      setRequer(false);
      load();
    } catch {
      // noop, UI simples
