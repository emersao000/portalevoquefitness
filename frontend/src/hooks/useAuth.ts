...
const SESSION_EXPIRY = 3 * 24 * 60 * 60 * 1000; // 3 dias — alinhado com expiração do Auth0
...
function readFromStorage(): AuthUser | null {
  ...
  // Lê de localStorage (persiste entre abas e após fechar o browser)
    ...
    const refresh = async () => {
      try {
        ...
        try {
          ...
          localStorage.setItem(AUTH_KEY, recordJson);
          const verify = localStorage.getItem(AUTH_KEY);
          if (verify) {
