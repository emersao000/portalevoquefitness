...
from ti.services.metrics import MetricsCalculator, STATUS_ABERTO, STATUS_EM_ATENDIMENTO, STATUS_AGUARDANDO, STATUS_CONCLUIDO, STATUS_EXPIRADO, STATUS_AGUARDANDO_TOTAL
...
_OVERVIEW_CACHE_TTL = 60  # segundos

_performance_cache: dict = {}
_PERFORMANCE_CACHE_TTL = 5 * 60  # 5 minutos (cálculo pesado, muda pouco)
...
def get_overview(
    ...
    # Cache key — não cacheia período customizado
    cache_key = f"{range}:{statuses}" if not (start_date and end_date) else None
    if cache_key:
        cached = _overview_cache.get(cache_key)
        if cached and (_time.monotonic() - cached["ts"]) < _OVERVIEW_CACHE_TTL:
            return cached["data"]
    ...
    if cache_key:
        _overview_cache[cache_key] = {"data": result, "ts": _time.monotonic()}
        # Limpar entradas antigas
        if len(_overview_cache) > 50:
            oldest = min(_overview_cache, key=lambda k: _overview_cache[k]["ts"])
    ...
    try:
        ...
        chamados_hoje = db.query(Chamado).filter(
            and_(
                ...
                ~Chamado.status.in_(STATUS_EXPIRADO)
            )
        ).count()
        ...
        chamados_em_atendimento = db.query(Chamado).filter(
            and_(
                ...
                Chamado.status.in_(STATUS_EM_ATENDIMENTO)
            )
        ).count()
        ...
        abertos_agora = db.query(Chamado).filter(
            and_(
                Chamado.status.in_(STATUS_ABERTO),
                Chamado.deletado_em.is_(None)
            )
        ).count()

        # Aguardando agora (sempre, não filtra por período) - inclui Expirado
        aguardando_agora = db.query(Chamado).filter(
            and_(
                Chamado.status.in_(STATUS_AGUARDANDO_TOTAL),
                Chamado.deletado_em.is_(None)
            )
        ).count()
