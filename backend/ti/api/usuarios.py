...
def excluir_usuario(user_id: int, db: Session = Depends(get_db)):
    try:
        ...
        # Guarda auth0_id e email ANTES de deletar do banco
        auth0_id = user.auth0_id
        email = user.email

        # 1. Deleta do banco local (Azure)
        delete_user(db, user_id)
        print(f"[DELETE USER] ✅ Usuário id={user_id} ({email}) deletado do banco local")

        # 2. Deleta do Auth0 (não falha a operação se der erro aqui)
        try:
            from auth0.management import get_auth0_client
            auth0_client = get_auth0_client()

            if auth0_id:
                # Deleta diretamente pelo auth0_id salvo no banco
                auth0_client.delete_user(auth0_id)
                print(f"[DELETE USER] ✅ Usuário deletado do Auth0 via auth0_id: {auth0_id}")
            else:
                # Fallback: busca pelo email no Auth0
                ...
                if auth0_user and auth0_user.get("user_id"):
                    ...
                    print(f"[DELETE USER] ✅ Usuário deletado do Auth0 via email: {email}")
                else:
                    print(f"[DELETE USER] ℹ️  Usuário {email} não encontrado no Auth0")
        except Exception as auth0_err:
            print(f"[DELETE USER] ⚠️  Erro ao deletar do Auth0 (banco já foi limpo): {auth0_err}")

        return {"ok": True, "message": f"Usuário id={user_id} excluído com sucesso"}
    except HTTPException:
        raise
    except Exception as e:
        import traceback
