...
def criar_chamado(db: Session, payload: ChamadoCreate) -> Chamado:
    ...
    # ── Guard anti-duplicata: bloqueia chamado idêntico criado nos últimos 10s ─
    from datetime import timedelta
    janela = now_brazil_naive() - timedelta(seconds=10)
    duplicata = db.query(Chamado).filter(
        Chamado.email == str(payload.email),
        Chamado.problema == payload.problema,
        Chamado.unidade == payload.unidade,
        Chamado.data_abertura >= janela,
        Chamado.deletado_em.is_(None),
    ).first()
    if duplicata:
        print(f"[CHAMADOS] ⚠️  Chamado duplicado bloqueado para {payload.email} "
              f"(id={duplicata.id}, criado em {duplicata.data_abertura})")
        return duplicata
