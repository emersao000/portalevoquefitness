...
def criar_usuario(db: Session, payload: UserCreate) -> UserCreatedOut:
    ...
    try:
        ...
        # ✅ Propaga o erro — não cria usuário local sem Auth0
        # Isso evita usuários "fantasma" no banco sem auth0_id
        raise ValueError(f"Falha ao criar usuário no Auth0: {auth0_error}")
...
def delete_user(db: Session, user_id: int) -> None:
    """
    Remove um usuário do banco de forma segura.
    Antes de deletar, nula todas as FKs nas tabelas relacionadas
    e remove registros que nao permitem NULL (ex: sessions).
    """
    try:
        User.__table__.create(bind=engine, checkfirst=True)
    except Exception:
    ...
    # 1. Descobrir DINAMICAMENTE todas as tabelas/colunas com FK apontando para user.id
    #    Isso garante que nunca vai falhar por uma tabela nova não listada manualmente
    try:
        fk_rows = db.execute(text("""
            ...
            WHERE kcu.REFERENCED_TABLE_NAME  = 'user'
              AND kcu.REFERENCED_COLUMN_NAME = 'id'
              AND kcu.TABLE_SCHEMA = DATABASE()
        """)).fetchall()
    except Exception as e:
        print(f"[DELETE USER] Aviso ao consultar FKs: {e}")
        fk_rows = []

    # Desabilita verificação de FK para evitar erros de dependência em cascata
    # (ex: chamado_agente → agentes_suporte → user)
    try:
        db.execute(text("SET FOREIGN_KEY_CHECKS = 0"))
    except Exception as e:
        print(f"[DELETE USER] Aviso ao desabilitar FK checks: {e}")

    for row in fk_rows:
        tabela, coluna, is_nullable = row[0], row[1], row[2]
        if is_nullable == "YES":
            # Coluna aceita NULL → apenas nullar a referência
            try:
                db.execute(
                    text(f"UPDATE `{tabela}` SET `{coluna}` = NULL WHERE `{coluna}` = :uid"),
                    {"uid": user_id},
                )
                print(f"[DELETE USER] Nullado: {tabela}.{coluna}")
            except Exception as e:
                print(f"[DELETE USER] Aviso ao nullar {tabela}.{coluna}: {e}")
        else:
            # Coluna NOT NULL → deletar o registro inteiro
            try:
                db.execute(
                    text(f"DELETE FROM `{tabela}` WHERE `{coluna}` = :uid"),
                    {"uid": user_id},
                )
                print(f"[DELETE USER] Deletado: {tabela}.{coluna}")
            except Exception as e:
                print(f"[DELETE USER] Aviso ao deletar {tabela}.{coluna}: {e}")

    # 2. Deletar o usuario
    db.delete(user)
    db.commit()

    # Reabilita verificação de FK após o commit
    try:
        db.execute(text("SET FOREIGN_KEY_CHECKS = 1"))
        db.commit()
    except Exception as e:
        print(f"[DELETE USER] Aviso ao reabilitar FK checks: {e}")

    print(f"[DELETE USER] Usuario id={user_id} excluido com sucesso.")
