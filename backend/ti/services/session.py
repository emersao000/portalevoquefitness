...
class SessionService:
    ...
    def renew_session(
        db: Session,
        session_token: str,
        extend_seconds: int = 259200,  # 3 days
    ) -> "SessionModel | None":
        """Renew (extend) an existing active session"""
        session = SessionService.get_session(db, session_token)
        if not session:
            return None
        if session.is_expired():
            ...
            db.commit()
            return None
        new_expires = datetime.utcnow() + timedelta(seconds=extend_seconds)
        session.access_token_expires_at = new_expires
